...
// HMAC secret key - stored only on the reader, never on the card
#define HMAC_KEY "SuperSecretRFIDKey2024"
#define HMAC_KEY_LEN 22

// Card memory layout:
// Block 4: first 16 bytes of HMAC tag
// Block 5: last 16 bytes of HMAC tag
// Block 6: counter (4 bytes little-endian)
// HMAC is computed over (UID + counter) so both must match to authenticate
#define HMAC_BLOCK_A 4
#define HMAC_BLOCK_B 5
#define COUNTER_BLOCK 6
...
// Counter database - mirrors counter stored on each card
...
// Compute HMAC over (UID + counter)
// Message = 4 bytes UID + 4 bytes counter = 8 bytes total

void computeExpectedHMAC(byte *outHash, uint32_t counter)
{
    byte msg[8];
...
// Read HMAC from card (blocks 4 and 5)
...
// Write HMAC to card (blocks 4 and 5)
...
// Read counter from card (block 6)
...
// Full authentication: counter match + HMAC(UID + counter) match

bool authenticate(int uidIndex)
{
    ...
    // Compute expected HMAC(UID + counter)
    byte expectedHash[32];
    computeExpectedHMAC(expectedHash, cardCounter);
    ...
    if (diff != 0)
    {
        Serial.println(F("HMAC MISMATCH"));
        return false;
    }

    // Both match - increment counter
    ...
    // Compute new HMAC for next use: HMAC(UID + newCounter)
    byte newHash[32];
    computeExpectedHMAC(newHash, newCounter);

    counterDB[uidIndex] = newCounter;

    // Re-read card to write new counter and new HMAC
    rfid.PICC_HaltA();
    rfid.PCD_StopCrypto1();
    delay(100);
    ...
    if (!writeCounterToCard(newCounter) || !writeHMACToCard(newHash))
    {
        Serial.println(F("Write failed - rolling back"));
        counterDB[uidIndex] = cardCounter;
        return false;
    }
...
void handleSerialCommand()
{
    ...
    {
        // Write counter=0 and HMAC(UID+0) to card
        byte hash[32];
        computeExpectedHMAC(hash, 0);

        if (writeCounterToCard(0) && writeHMACToCard(hash))
            Serial.println(F("Card enrolled"));
        else
            Serial.println(F("Enrollment failed - hold card closer"));
    }
...
void setup()
{
    ...
    Serial.println(F("To enroll: ADD <uid> then ENROLL"));
}
...
void loop()
{
    ...
    if (authenticate(uidIndex))
    {
        Serial.println(F("HMAC + COUNTER OK"));
